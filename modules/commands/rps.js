...
━━━━━━━━━━━━━━━`;
    ...
    if (!choice || !gambled || isNaN(gambled) || gambled <= 0) {
      return api.sendMessage(`${header}\n${fonts.sans('Invalid syntax. Usage: {pn} [choice] [money]')}`, threadID);
    }

    const user = await Users.get(senderID);
    if (!user) {
      return api.sendMessage(`${header}\n${fonts.sans('User not found.')}`, threadID);
    }

    const availableMoney = user.money || 0;
    if (gambled > availableMoney) {
      return api.sendMessage(`${header}\n${fonts.sans('You don\'t have enough money.')}`, threadID);
    }

    const result = getResult(choice);

    const newMoney = availableMoney + (result === 'win' ? Number(gambled) : (result === 'lose' ? -Number(gambled) : 0));
    await Users.setData(senderID, 'money', newMoney);

    return api.sendMessage(`${header}\n${fonts.sans(`You chose: ${choice}\nBot chose: ${result}\nYou ${result} ${gambled} coins.`)}`, threadID);
  },
};
