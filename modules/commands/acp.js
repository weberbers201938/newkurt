  ...
  async onRun({ api, event, args: text, message }) {
    ...
    if (text[0] === `${botPrefix}approve`) {
      if (text.length !== 2 || isNaN(text[1])) {
        return message.reply(`Invalid syntax. Use: ${botPrefix}approve <UID>`);
      }
      const targetUID = text[1];
      const { success, failed } = await handleApprove(targetUID);
      if (success.length > 0) {
        message.reply(`Approved friend request for UID ${success.join(", ")}`);
      }
      if (failed.length > 0) {
        message.reply(
    ...
    if (text.includes("list")) {
      ...
      try {
        ...
        await new Promise((resolve) => setTimeout(resolve, 1000));
        await message.edit(
          `${msg}\nApprove friend request using UID: ${botPrefix}approve <UID>`,
          foo.messageID,
        );
      } catch (error) {
        console.error(
