...
━━━━━━━━━━━━━━━`;
    ...
    const [choice, gambled] = args;

    if (!choice || !gambled) {
      return api.sendMessage(`${header}\n${fonts.sans("Please provide both a choice (head or tail) and the amount of money to gamble.")}`, threadID);
    }

    if (!isValidChoice(choice)) {
      return api.sendMessage(`${header}\n${fonts.sans("Invalid choice. Please choose either 'head' or 'tail'.")}`, threadID);
    }

    const user = await Users.get(senderID);
    if (!user) {
      return api.sendMessage(`${header}\n${fonts.sans("User data not found.")}`, threadID);
    }

    const userMoney = user.money || 0;

    if (userMoney < gambled) {
      return api.sendMessage(`${header}\n${fonts.sans("You don't have enough money to gamble.")}`, threadID);
    }
